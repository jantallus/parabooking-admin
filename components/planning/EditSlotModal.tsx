  ...
  const handleSaveNote = async () => {
    ...
    if ((groupSize > 1 || isManual || groupRootSlots.length > 1) && !groupLocked) {
      ...
      displayDistribution.slotsToUse.forEach((baseSlot, index) => {
        ...
        else if (namesList.length > 0) { const booker = namesList[0]; passengerTitle = index === 0 ? booker : (namesList[index] ? `${namesList[index]} (${booker})` : `${booker} (${booker})`); }
        else { passengerTitle = groupSize > 1 ? `${effectiveTitle} (${effectiveTitle})` : (effectiveTitle || ''); }
