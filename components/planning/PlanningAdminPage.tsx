...
export default function PlanningAdmin() {
  ...
  const calendarEvents = useMemo(() => {
    return appointments.map(a => {
      ...
      const isTentative = !!(a.payment_data as { tentative?: boolean } | null)?.tentative;
  ...
  const monitorSlotTotals = useMemo(() => {
    ...
    for (const ev of calendarEvents) {
      ...
      if (pd.complement_payment_type && pd.complement_encaisseur_id && compTotal > 0) {
        add(effectiveEncId, ep.start_time, flightOnly);
        add(String(pd.complement_encaisseur_id), ep.start_time, compTotal);
      } else {
        add(effectiveEncId, ep.start_time, ep.price_cents ?? 0);
      }
    }
    return map;
  ...
  const resourceLabelContent = useCallback((arg: { resource: { id: string; title: string } }) => (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', gap: '4px', minWidth: 0 }}>
      ...
      <button
        ...
        style={{ flexShrink: 0, background: 'none', border: 'none', cursor: 'pointer', opacity: 0.4, fontSize: '11px', padding: '0 4px 0 8px', lineHeight: '1', color: 'inherit' }}
        onMouseEnter={e => (e.currentTarget.style.opacity = '1')}
